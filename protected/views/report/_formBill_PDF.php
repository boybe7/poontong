...
foreach ($buymaterial as $key => $value) {
        ...
        $html .= '<table border=0><tr style="font-weight:bold;font-size:16px;"><td width="50%">เลขที่ : '.$value['bill_no'].'</td>';
        $html .= '<td width="50%" style="text-align:right">วันที่ : '.$this->renderDate($value["buy_date"]).'</td></tr>';
        $html .= '<tr style="font-weight:bold;font-size:16px;"><td colspan=2>ลูกค้า : '.$value["name"].'</td></tr></table>';
