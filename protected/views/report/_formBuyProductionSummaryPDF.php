...
$pdf->SetTitle('รายงานสรุปซื้อขาย(บด)');
...
$html = "";
$html .= '<center><div class="header" style="text-align:center;"><h4>รายงานสรุปซื้อขาย(บด) ปี '.$year.'</h4></div></center>';
    ...
    $materialSell = Yii::app()->db->createCommand()
        ...
        $html .= '<tr>';
            ...
            $colwidth = round(80/($num_material_buy+$num_material_sell));
            $colheader = $num_material_buy*$colwidth;
            $html .= '<td colspan="'.$num_material_buy.'" width="'.$colheader.'%" style="text-align:center;font-weight:bold;background-color: #def7d7;">รายการซื้อ (กก.)</td>';
            $colheader = $num_material_sell*$colwidth;
            $html .= '<td colspan="'.$num_material_sell.'" width="'.$colheader.'%" style="text-align:center;font-weight:bold;background-color: #ffeec1;">รายการบด (กก.)</td>';
            $html .= '<td rowspan="2" width="10%" style="width:10%;text-align:center;font-weight:bold;background-color: #c3c8c2;">บด-ซื้อ (กก.)</td>';
        ...
        $html .= '<tr>';
            ...
            foreach ($materialBuy as $key => $value) {
                $html .= '<td  width="'.$colwidth.'%" style="text-align:center;font-weight:bold;background-color: #afd7a3;">'.Material::model()->FindByPk($value["material_id"])->name.'</td>';
                //$material_id[] = $value["material_id"];
            }
            $html .= '<td  width="'.$colwidth.'%" style="text-align:center;font-weight:bold;background-color: #def7d7;">รวมน้ำหนัก (กก.)</td>';

            foreach ($materialSell as $key => $value) {
                $html .= '<td  width="'.$colwidth.'%" style="text-align:center;font-weight:bold;background-color: #ffe295;">'.Material::model()->FindByPk($value["material_id"])->name.'</td>';
                //$material_id[] = $value["material_id"];
            }
            $html .= '<td  width="'.$colwidth.'%" style="text-align:center;font-weight:bold;background-color: #def7d7;">รวมน้ำหนัก (กก.)</td>';
        ...
        for ($i=1; $i <= 12; $i++) {
            ...
            $html .= '<tr>';
                ...
                foreach ($materialBuy as $key => $value) {
                    ...
                    $html .= '<td  width="'.$colwidth.'%" style="text-align:right;background-color:'.$bgcolor.'">'.$amount.'</td>';      
                }   
                $html .= '<td style="text-align:right;background-color:'.$bgcolor.'">'.number_format($sumbuy,2).'</td>';
                $sumsell = 0;
                foreach ($materialSell as $key => $value) {
                ...
                $html .= '<td style="text-align:right;background-color:'.$bgcolor.'">'.number_format($sumsell,2).'</td>';
                $html .=  '<td style="text-align:right;background-color:'.$bgcolor.'">'.number_format($sumsell-$sumbuy,2).'</td>';
            $html .= '</tr>';
        }
        ...
        $html .= '<tr>';
                ...
                foreach ($materialBuy as $key => $value) {
                    ...
                    $amount = empty($mbuy[0]['amount']) ? "-" : number_format($mbuy[0]['amount'],2);  
                    $sumbuy += $mbuy[0]['amount'];      
                    $html .= '<td style="text-align:right;font-weight:bold;background-color:#c3c8c2">'.$amount.'</td>';     
                }   
                $html .= '<td style="text-align:right;font-weight:bold;background-color:#c3c8c2">'.number_format($sumbuy,2).'</td>';  
                $sumsell = 0;
                foreach ($materialSell as $key => $value) {
                    $msell = Yii::app()->db->createCommand()
                            ->select('sum(amount) as amount')
                ...
                $html .= '<td style="text-align:right;font-weight:bold;background-color:#c3c8c2">'.number_format($sumsell,2).'</td>';
                $html .= '<td rowspan="2" style="text-align:right;font-weight:bold;background-color:#c3c8c2">'.number_format($sumsell-$sumbuy,2).'</td>';  
        $html .= '</tr>';
        $html .= '<tr>';
                $html .= '<td style="text-align:center;font-weight:bold;background-color:#e4e4e4"></td>';
                $ncol = count($materialBuy);
                $colheader = $ncol*$colwidth;
                $html .= '<td width="'.$colheader.'%" colspan="'.$ncol.'" style="text-align:center;font-weight:bold;background-color:#9ccffb">รวมซื้อ</td>';
                
                $html .= '<td width="'.$colwidth.'%" style="text-align:right;font-weight:bold;background-color:#9ccffb"><u>'.number_format($sumbuy,2).'</u></td>';
                $ncol = count($materialSell);
                $colheader = $ncol*$colwidth;
                $html .= '<td width="'.$colheader.'%"  colspan="'.$ncol.'" style="text-align:center;font-weight:bold;background-color:#9ccffb">รวมขาย</td>';
                
                $html .= '<td width="'.$colwidth.'%" style="text-align:right;font-weight:bold;background-color:#9ccffb"><u>'.number_format($sumsell,2).'</u></td>';
