...
class SellMaterialController extends Controller
{
	/**
	...
	{
		$model=new SellMaterial;

		//running invoice_no
            $year = date('Y');
            $site = Yii::app()->user->isAdmin() ? 1 : Yii::app()->user->getSite();
            $sql = 'SELECT MAX(bill_no) as max_no FROM sell_material  WHERE site_id='.$site.' AND YEAR(sell_date)='.$year;
        ...
        $temp = SellMaterialDetailTemp::model()->findAll("flag=1 AND user_id=".Yii::app()->user->ID);
        if(empty($temp))
        {
        	$temp = new SellMaterialDetailTemp;
        	$temp->sell_id = 0;
		...
		if(isset($_POST['SellMaterial']))
		{

			//header('Content-type: text/plain');
			$model->attributes=$_POST['SellMaterial'];
			$model->note = $_POST['SellMaterial']['note'];
			...
			if($model->save())
			{
				$temp = SellMaterialDetailTemp::model()->findAll("user_id=:user_id AND flag is NULL", [":user_id" =>Yii::app()->user->ID]);

				foreach ($temp as $key => $value) {
					$modelDetail = new SellMaterialDetail;
        ...
        {
        	//header('Content-type: text/plain'); 
        	SellMaterialDetailTemp::model()->deleteAll("user_id=:user_id AND flag is NULL", [":user_id" =>Yii::app()->user->ID]);
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			//SellMaterialDetail::model()->deleteAll("sell_id=:sell_id", [":sell_id" =>$id]);
			$details = SellMaterialDetail::model()->findAll("sell_id=:sell_id", [":sell_id" =>$id]);
	...
	{
		$model=new SellMaterial('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SellMaterial']))
			$model->attributes=$_GET['SellMaterial'];
	...
	{
		$model=SellMaterial::model()->findByPk($id);
		if($model===null)
	...
	public function actionCreateItemDetailTemp()
	{
		...
		$model = new SellMaterialDetailTemp;
	...
	{
		$model = SellMaterialDetailTemp::model()->findByPk($id);
		$model->delete();
    ...
    {
	    $es = new EditableSaver('SellMaterialDetailTemp');
	    try {
	    	...
	    	{
	    		$model = SellMaterialDetailTemp::model()->findByPk($_POST['pk']);
	    		$_POST['name']='price_net';	
	    		$_POST['value'] = $model->price_unit*$model->amount;
	    		$es->update();
	    	}	

	    	if($_POST['name']=='weight_in' || $_POST['name']=='weight_out')
	    	{
	    		$model = SellMaterialDetailTemp::model()->findByPk($_POST['pk']);
	    		$_POST['name']='amount';
	    		
	    		$amount = $model->weight_in - $model->weight_out;
	    		$_POST['value'] = $amount;
	    		$es->update();
	...
	{
		$model = SellMaterialDetail::model()->findByPk($id);
    ...
    {
	    $es = new EditableSaver('SellMaterialDetail');
	    try {
	    	$modelOld = SellMaterialDetail::model()->findByPk($_POST['pk']);

	    	$es->update();

	    	if($_POST['name']=='price_unit' || $_POST['name']=='amount')
	    	{
	    		$model = SellMaterialDetail::model()->findByPk($_POST['pk']);
	    		$_POST['name']='price_net';	
	    		$_POST['value'] = $model->price_unit*$model->amount;
	    		$es->update();
	    	}	
	    	...
	    	$modelNew = SellMaterialDetail::model()->findByPk($_POST['pk']);
